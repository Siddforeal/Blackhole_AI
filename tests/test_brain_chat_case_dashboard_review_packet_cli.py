...
def test_brain_chat_case_dashboard_review_packet_cli_reads_explicit_session(tmp_path):
    session_file = tmp_path / "brain-chat-session.json"
    output_file = tmp_path / "review-packet.md"
    json_output = tmp_path / "review-packet.json"
    ...
    assert "Brain Chat Case Dashboard Review Packet" in result.output
    assert "blocked-review" in result.output
    assert "Dashboard state is not reportable by itself." in result.output
...
def test_brain_chat_case_dashboard_review_packet_cli_defaults_to_current_directory(tmp_path, monkeypatch):
    session_file = tmp_path / "brain-chat-session.json"
    session_file.write_text(json.dumps(_session_data()), encoding="utf-8")
    monkeypatch.chdir(tmp_path)

    result = runner.invoke(app, ["brain-chat-case-dashboard-review-packet"])
...
def test_brain_chat_case_dashboard_review_packet_cli_missing_file_returns_error(tmp_path):
    result = runner.invoke(
        app,
        [
            "brain-chat-case-dashboard-review-packet",
