...
def test_validation_plan_step_review_gate_cli_blocks_without_effective_approval(tmp_path):
    session_file = _write_session(tmp_path)
    decision_file = _write_decision(tmp_path, "approved")
    json_output = tmp_path / "step-review-gate.json"
    ...
    assert "Brain Chat Validation Plan Step Review Gate" in result.output
    assert "blocked-pending-approved-validation-plan" in result.output
...
def test_validation_plan_step_review_gate_cli_reviews_ready_plan(tmp_path):
    session_file = _write_session(tmp_path)
    ...
    assert "Allowed for manual review" in result.output
    assert "Needs scope check" in result.output
    assert "[allowed-for-manual-review]" in result.output
    assert "[needs-scope-check]" in result.output


def test_validation_plan_step_review_gate_cli_missing_decision_file_errors(tmp_path):
    session_file = _write_session(tmp_path)
...
def test_validation_plan_step_review_gate_cli_invalid_decision_errors(tmp_path):
    session_file = _write_session(tmp_path)
