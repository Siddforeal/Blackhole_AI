...
def test_validation_step_approval_decision_cli_does_not_approve_blocked_request(tmp_path):
    ...
    json_output = tmp_path / "step-decision-result.json"
    ...
    assert "Brain Chat Validation Step Approval Decision" in result.output
    assert "approved" in result.output
    assert "Effective step approval granted" in result.output
...
def test_validation_step_approval_decision_cli_tracks_changes_requested(tmp_path):
    session_file = _write_session(tmp_path)
    status_file = _write_status(tmp_path, {label: "collected" for label in LABELS})
    approval_decision_file = _write_approval_decision(tmp_path, "approved")
    step_decision_file = _write_step_decision(tmp_path, "changes-requested")
    ...
    assert "Update validation step review state and reviewer notes." in result.output
    assert "Effective step approval granted" in result.output
    assert "False" in result.output


def test_validation_step_approval_decision_cli_missing_step_decision_file_errors(tmp_path):
    session_file = _write_session(tmp_path)
    approval_decision_file = _write_approval_decision(tmp_path, "approved")
...
def test_validation_step_approval_decision_cli_missing_approval_decision_file_errors(tmp_path):
    session_file = _write_session(tmp_path)
    step_decision_file = _write_step_decision(tmp_path, "approved")
...
def test_validation_step_approval_decision_cli_invalid_step_decision_errors(tmp_path):
    session_file = _write_session(tmp_path)
    approval_decision_file = _write_approval_decision(tmp_path, "approved")
