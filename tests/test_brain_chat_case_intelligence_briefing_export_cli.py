...
def test_briefing_export_cli_exports_blocked_briefing(tmp_path):
    ...
    json_output = tmp_path / "briefing.json"
    markdown_output = tmp_path / "briefing.md"
    ...
    assert "Brain Chat Case Intelligence Briefing Export" in result.output
    assert "Briefing status" in result.output
...
def test_briefing_export_cli_accepts_custom_questions_file(tmp_path):
    session_file = _write_session(tmp_path)
    status_file = _write_status(tmp_path)
...
def test_briefing_export_cli_no_artifacts_is_safe(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)

    result = runner.invoke(app, ["brain-chat-case-intelligence-briefing-export"])
...
def test_briefing_export_cli_missing_questions_file_errors(tmp_path):
    session_file = _write_session(tmp_path)
...
def test_briefing_export_cli_invalid_questions_file_errors(tmp_path):
    session_file = _write_session(tmp_path)
    questions_file = tmp_path / "questions.json"
...
def test_briefing_export_cli_invalid_step_decision_errors(tmp_path):
    session_file = _write_session(tmp_path)
