...
def test_request_contract_loads_blueprint_without_execution() -> None:
    request = ScopedAdapterRequest.from_blueprint_artifact(_blueprint())

    assert request.implementation_blueprint_id == "SIB-ERR-SEP-RCP-FEG-ASR-RSR-SAER-AFC-ADP-RSM-EG-CP-AD-AP-001"
    assert request.blueprint_artifact_kind == "case_intake_brain_handoff_scoped_adapter_implementation_blueprint"
    assert request.adapter_family == "curl"
    assert request.command_family == "curl"
    assert request.reviewed_method == "GET"
    assert request.reviewed_scheme == "https"
...
def test_request_contract_serializes_safety_flags() -> None:
    data = ScopedAdapterRequest.from_blueprint_artifact(_blueprint()).to_dict()
...
def test_prepared_command_contract_is_preview_only() -> None:
    request = ScopedAdapterRequest.from_blueprint_artifact(_blueprint())
