...
from bugintel.core.brain_chat_human_case_review_decision_gate import (
    build_human_case_review_decision_gate,
)
...
def _request(summary, upstream_decision="approved-for-human-case-review"):
...
def _import(summary, decision="changes-requested", upstream_decision="approved-for-human-case-review"):
    request = _request(summary, upstream_decision=upstream_decision)
    return import_human_case_review_decision_data(
        request,
        {
            "decision": decision,
            "reason": "Local case-review decision.",
            "reviewer": "local-reviewer",
        },
    )


def test_decision_gate_changes_requested():
    imported = _import(_blocked_summary(), "changes-requested")
    gate = build_human_case_review_decision_gate(imported)
    data = gate.to_dict()

    assert data["kind"] == "brain_chat_human_case_review_decision_gate"
    assert data["decision"] == "changes-requested"
    assert data["decision_gate_status"] == "changes-requested"
    assert data["next_local_planning_gate_ready"] is False
...
def test_decision_gate_rejected():
    imported = _import(_blocked_summary(), "rejected")
    gate = build_human_case_review_decision_gate(imported)
    data = gate.to_dict()

    assert data["decision_gate_status"] == "rejected"
    assert data["next_local_planning_gate_ready"] is False
    assert any("Stop this human case-review path" in item for item in data["allowed_local_next_steps"])


def test_decision_gate_ready_for_next_local_planning_gate():
    imported = _import(_ready_summary(), "approved-for-next-local-planning-gate")
    gate = build_human_case_review_decision_gate(imported)
    data = gate.to_dict()

    assert data["decision_gate_status"] == "ready-for-next-local-planning-gate"
    assert data["next_local_planning_gate_ready"] is True
...
def test_decision_gate_blocks_unsafe_import():
    imported = _import(_ready_summary(), "approved-for-next-local-planning-gate")
    unsafe = replace(imported, runtime_execution_allowed=True)

    gate = build_human_case_review_decision_gate(unsafe)
    data = gate.to_dict()

    assert data["decision_gate_status"] == "blocked-pending-safe-human-case-review-decision-import"
    assert data["next_local_planning_gate_ready"] is False
...
def test_decision_gate_markdown_is_readable():
    ...
    assert "# Brain Chat Human Case Review Decision Gate" in markdown
