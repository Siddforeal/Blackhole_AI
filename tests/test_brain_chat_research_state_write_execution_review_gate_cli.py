...
def _preview():
    ...
    return {
        "kind": "brain_chat_research_state_local_write_packet_preview",
        "source": "brain-chat-research-state-local-write-packet-preview",
        "target_name": "demo-target",
        "preview_status": "ready-for-write-execution-review-gate",
...
def test_write_execution_review_gate_cli_writes_json(tmp_path):
    ...
    result = runner.invoke(
        ...
        [
            ...
            "--local-write-packet-preview-file",
            str(preview_file),
            "--json-output",
            str(output_file),
        ],
    ...
    assert "Write Execution Review Gate" in result.output
    assert "Saved write execution review gate JSON" in result.output

    gate = json.loads(output_file.read_text(encoding="utf-8"))
    assert gate["kind"] == "brain_chat_research_state_write_execution_review_gate"
...
def test_write_execution_review_gate_cli_prints_json_without_output(tmp_path):
    preview_file = tmp_path / "local-write-preview.json"
    preview_file.write_text(json.dumps(_preview()), encoding="utf-8")
    ...
    assert result.exit_code == 0, result.output
    assert '"kind": "brain_chat_research_state_write_execution_review_gate"' in result.output
    assert '"write_execution_review_ready": true' in result.output
...
def test_write_execution_review_gate_cli_missing_preview_file_exits(tmp_path):
    result = runner.invoke(
        app,
        [
            "brain-chat-research-state-write-execution-review-gate",
    ...
    assert "Local write packet preview JSON not found" in result.output
