...
def _packet():
    ...
    return {
        "kind": "brain_chat_research_state_write_execution_decision_packet",
        "source": "brain-chat-research-state-write-execution-decision-packet",
        "target_name": "demo-target",
        "decision_status": "ready-for-local-write-execution-packet",
...
def test_local_write_execution_packet_cli_writes_json(tmp_path):
    ...
    result = runner.invoke(
        ...
        [
            ...
            "--write-execution-decision-packet-file",
            str(packet_file),
            "--json-output",
            str(output_file),
        ],
    ...
    assert "Local Write Execution Packet" in result.output
    assert "Saved local write execution packet JSON" in result.output

    packet = json.loads(output_file.read_text(encoding="utf-8"))
    assert packet["kind"] == "brain_chat_research_state_local_write_execution_packet"
...
def test_local_write_execution_packet_cli_prints_json_without_output(tmp_path):
    packet_file = tmp_path / "write-execution-decision-packet.json"
    packet_file.write_text(json.dumps(_packet()), encoding="utf-8")
    ...
    result = runner.invoke(
        ...
        [
            "brain-chat-research-state-local-write-execution-packet",
            "--execution-decision-packet",
            str(packet_file),
        ],
    )

    assert result.exit_code == 0, result.output
    assert '"kind": "brain_chat_research_state_local_write_execution_packet"' in result.output
    assert '"local_write_execution_packet_ready": true' in result.output
...
def test_local_write_execution_packet_cli_missing_packet_file_exits(tmp_path):
    result = runner.invoke(
        app,
        [
            "brain-chat-research-state-local-write-execution-packet",
    ...
    assert "Write execution decision packet JSON not found" in result.output
