...
def test_human_case_review_decision_gate_cli_changes_requested(tmp_path):
    ...
    json_output = tmp_path / "human-case-review-decision-gate.json"
    markdown_output = tmp_path / "human-case-review-decision-gate.md"
    ...
    assert "Brain Chat Human Case Review Decision Gate" in result.output
...
def test_human_case_review_decision_gate_cli_rejected(tmp_path):
    session_file = _write_session(tmp_path)
    upstream_decision_file = _write_upstream_human_decision(tmp_path)
...
def test_human_case_review_decision_gate_cli_blocks_approval_when_request_blocked(tmp_path):
    session_file = _write_session(tmp_path)
    upstream_decision_file = _write_upstream_human_decision(tmp_path)
...
def test_human_case_review_decision_gate_cli_missing_decision_file_errors(tmp_path):
    upstream_decision_file = _write_upstream_human_decision(tmp_path)
...
def test_human_case_review_decision_gate_cli_missing_upstream_file_errors(tmp_path):
...
def test_human_case_review_decision_gate_cli_invalid_decision_errors(tmp_path):
    session_file = _write_session(tmp_path)
    upstream_decision_file = _write_upstream_human_decision(tmp_path)
