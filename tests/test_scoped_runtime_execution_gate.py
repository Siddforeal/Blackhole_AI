...
def test_execution_gate_bundle_handoff_checklist_summary_receipt_accepts_summary_without_execution(tmp_path) -> None:
    bundle_dir = _write_bundle(tmp_path)
    verification = verify_scoped_runtime_execution_gate_bundle(bundle_dir)
    review_packet = review_scoped_runtime_execution_gate_bundle_verification(
        verification.to_dict(),
    ...
    assert data["kind"] == "scoped_runtime_execution_gate_bundle_handoff_checklist_summary_receipt"
    assert data["receipt_status"] == "accepted-local-bundle-handoff-checklist-summary-receipt-no-execution"
    assert data["receipt_state"] == "receipt_local_only"
    assert data["received_by"] == "human-reviewer"
    assert data["summary_status"] == "summarized-local-bundle-handoff-checklist-no-execution"
    assert data["checklist_status"] == "passed-local-bundle-handoff-checklist-no-execution"
    assert data["handoff_status"] == "ready-local-bundle-handoff-no-execution"
...
def test_execution_gate_bundle_handoff_checklist_summary_receipt_blocks_failed_summary(tmp_path) -> None:
    bundle_dir = _write_bundle(tmp_path, include_markdown=False)
    verification = verify_scoped_runtime_execution_gate_bundle(bundle_dir)
    review_packet = review_scoped_runtime_execution_gate_bundle_verification(
        verification.to_dict(),
    ...
    data = build_scoped_runtime_execution_gate_bundle_handoff_checklist_summary_receipt(
        summary.to_dict(),
        received_by="human-reviewer",
        receipt_note="Blocked receipt because summary is blocked.",
    ).to_dict()

    assert data["receipt_status"] == "blocked-local-bundle-handoff-checklist-summary-receipt"
    assert data["summary_status"] == "blocked-local-bundle-handoff-checklist-summary"
    assert data["final_handoff_outcome"] == "blocked-local-bundle-handoff-checklist-summary-receipt"
    assert any("not summarized-local-bundle-handoff-checklist-no-execution" in item for item in data["blocking_findings"])


def test_execution_gate_bundle_handoff_checklist_summary_receipt_markdown_is_human_readable_and_safe(tmp_path) -> None:
    bundle_dir = _write_bundle(tmp_path)
    verification = verify_scoped_runtime_execution_gate_bundle(bundle_dir)
    review_packet = review_scoped_runtime_execution_gate_bundle_verification(
        verification.to_dict(),
    ...
    assert "# Scoped Runtime Execution Gate Bundle Handoff Checklist Summary Receipt" in markdown
    assert "Receipt status: `accepted-local-bundle-handoff-checklist-summary-receipt-no-execution`" in markdown
    assert "Receipt state: `receipt_local_only`" in markdown
    assert "Received by: `human-reviewer`" in markdown
    assert "Final handoff outcome: `ready-for-future-local-review-no-execution`" in markdown
    assert "Required checks: `11`" in markdown
    assert "Failed checks: `0`" in markdown
    assert "Network requests allowed: `false`" in markdown
    assert "Tool execution allowed: `false`" in markdown
...
def test_execution_gate_bundle_handoff_receipt_archive_manifest_accepts_receipt_without_execution(tmp_path) -> None:
    bundle_dir = _write_bundle(tmp_path)
    verification = verify_scoped_runtime_execution_gate_bundle(bundle_dir)
    review_packet = review_scoped_runtime_execution_gate_bundle_verification(
        verification.to_dict(),
    ...
    assert data["kind"] == "scoped_runtime_execution_gate_bundle_handoff_receipt_archive_manifest"
    ...
    assert data["archived_by"] == "human-reviewer"
    assert data["receipt_status"] == "accepted-local-bundle-handoff-checklist-summary-receipt-no-execution"
    assert data["final_handoff_outcome"] == "ready-for-future-local-review-no-execution"
    assert data["required_check_count"] == 11
    assert data["passed_check_count"] == 11
    assert data["failed_check_count"] == 0
    assert len(data["upstream_artifact_chain"]) == 7
    assert data["archived_artifact_ids"]["receipt_id"] == data["receipt_id"]
    assert data["archived_artifact_ids"]["summary_id"] == data["summary_id"]
    assert data["archived_artifact_ids"]["checklist_id"] == data["checklist_id"]
    assert data["adapter_execution_state"] == "not_executed"
    assert data["can_execute_now"] is False
    assert data["execution_allowed"] is False
    assert data["runtime_execution_allowed"] is False
...
def test_execution_gate_bundle_handoff_receipt_archive_manifest_blocks_failed_receipt(tmp_path) -> None:
    bundle_dir = _write_bundle(tmp_path, include_markdown=False)
    verification = verify_scoped_runtime_execution_gate_bundle(bundle_dir)
    review_packet = review_scoped_runtime_execution_gate_bundle_verification(
        verification.to_dict(),
    ...
    data = build_scoped_runtime_execution_gate_bundle_handoff_receipt_archive_manifest(
        receipt.to_dict(),
        archived_by="human-reviewer",
        archive_note="Blocked archive because receipt is blocked.",
    ).to_dict()

    assert data["archive_status"] == "blocked-local-bundle-handoff-receipt-archive-manifest"
    assert data["receipt_status"] == "blocked-local-bundle-handoff-checklist-summary-receipt"
    assert any("not accepted-local-bundle-handoff-checklist-summary-receipt-no-execution" in item for item in data["blocking_findings"])


def test_execution_gate_bundle_handoff_receipt_archive_manifest_markdown_is_human_readable_and_safe(tmp_path) -> None:
    bundle_dir = _write_bundle(tmp_path)
    verification = verify_scoped_runtime_execution_gate_bundle(bundle_dir)
    review_packet = review_scoped_runtime_execution_gate_bundle_verification(
        verification.to_dict(),
    ...
    markdown = archive.to_markdown()

    assert "# Scoped Runtime Execution Gate Bundle Handoff Receipt Archive Manifest" in markdown
    assert "Archive status: `archived-local-bundle-handoff-receipt-manifest-no-execution`" in markdown
    assert "Archive state: `archive_manifest_local_only`" in markdown
    assert "Archived by: `human-reviewer`" in markdown
    assert "Final handoff outcome: `ready-for-future-local-review-no-execution`" in markdown
    assert "scoped_runtime_execution_gate_bundle_handoff_checklist_summary_receipt" in markdown
    assert "Required checks: `11`" in markdown
    assert "Failed checks: `0`" in markdown
    assert "Network requests allowed: `false`" in markdown
    assert "Tool execution allowed: `false`" in markdown
...
def test_execution_gate_bundle_handoff_receipt_archive_manifest_verification_accepts_archive_without_execution(tmp_path) -> None:
    bundle_dir = _write_bundle(tmp_path)
    verification = verify_scoped_runtime_execution_gate_bundle(bundle_dir)
    review_packet = review_scoped_runtime_execution_gate_bundle_verification(
        verification.to_dict(),
    ...
    verified = verify_scoped_runtime_execution_gate_bundle_handoff_receipt_archive_manifest(
        archive.to_dict(),
    ...
    assert data["kind"] == "scoped_runtime_execution_gate_bundle_handoff_receipt_archive_manifest_verification"
    ...
    assert data["verified_by"] == "human-reviewer"
    assert data["archive_status"] == "archived-local-bundle-handoff-receipt-manifest-no-execution"
    assert data["archive_state"] == "archive_manifest_local_only"
    assert data["receipt_status"] == "accepted-local-bundle-handoff-checklist-summary-receipt-no-execution"
    assert data["final_handoff_outcome"] == "ready-for-future-local-review-no-execution"
...
def test_execution_gate_bundle_handoff_receipt_archive_manifest_verification_blocks_failed_archive(tmp_path) -> None:
    bundle_dir = _write_bundle(tmp_path, include_markdown=False)
    verification = verify_scoped_runtime_execution_gate_bundle(bundle_dir)
    review_packet = review_scoped_runtime_execution_gate_bundle_verification(
        verification.to_dict(),
    ...
    data = verify_scoped_runtime_execution_gate_bundle_handoff_receipt_archive_manifest(
        archive.to_dict(),
        verified_by="human-reviewer",
    ...
    assert data["archive_status"] == "blocked-local-bundle-handoff-receipt-archive-manifest"
    assert any("not archived-local-bundle-handoff-receipt-manifest-no-execution" in item for item in data["blocking_findings"])


def test_execution_gate_bundle_handoff_receipt_archive_manifest_verification_markdown_is_human_readable_and_safe(tmp_path) -> None:
    bundle_dir = _write_bundle(tmp_path)
    verification = verify_scoped_runtime_execution_gate_bundle(bundle_dir)
    review_packet = review_scoped_runtime_execution_gate_bundle_verification(
        verification.to_dict(),
    ...
    markdown = verified.to_markdown()

    assert "# Scoped Runtime Execution Gate Bundle Handoff Receipt Archive Manifest Verification" in markdown
