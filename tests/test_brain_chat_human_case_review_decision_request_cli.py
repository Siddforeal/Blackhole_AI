...
def test_human_case_review_decision_request_cli_blocks_premature_request(tmp_path):
    ...
    json_output = tmp_path / "human-case-review-decision-request.json"
    markdown_output = tmp_path / "human-case-review-decision-request.md"
    ...
    assert "Brain Chat Human Case Review Decision Request" in result.output
    assert "blocked-pending-packet-review-gate" in result.output
...
def test_human_case_review_decision_request_cli_changes_requested(tmp_path):
    session_file = _write_session(tmp_path)
...
def test_human_case_review_decision_request_cli_rejected(tmp_path):
    session_file = _write_session(tmp_path)
...
def test_human_case_review_decision_request_cli_no_artifacts_keeps_request_blocked(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 0
    assert "blocked-pending-packet-review-gate" in result.output
    assert "Human case review decision ready" in result.output
    assert "False" in result.output


def test_human_case_review_decision_request_cli_missing_decision_file_errors(tmp_path):
    result = runner.invoke(
        app,
        [
            "brain-chat-human-case-review-decision-request",
...
def test_human_case_review_decision_request_cli_invalid_decision_errors(tmp_path):
    session_file = _write_session(tmp_path)
