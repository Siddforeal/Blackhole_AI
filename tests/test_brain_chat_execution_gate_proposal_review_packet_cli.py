...
def test_execution_gate_proposal_review_cli_blocks_without_effective_step_approval(tmp_path):
    ...
    json_output = tmp_path / "execution-gate-review.json"
    ...
    assert "Brain Chat Execution Gate Proposal Review Packet" in result.output
    assert "blocked-pending-effective-step-approval" in result.output
    assert "Effective validation-step approval is not granted." in result.output
...
def test_execution_gate_proposal_review_cli_blocks_when_scope_check_needed(tmp_path):
    session_file = _write_session(tmp_path)
    ...
    assert result.exit_code == 0
    assert "blocked-pending-effective-step-approval" in result.output
    assert "Proposal is blocked pending effective step approval." in result.output
    assert "Execution gate created" in result.output
    assert "false" in result.output


def test_execution_gate_proposal_review_cli_missing_step_decision_file_errors(tmp_path):
    session_file = _write_session(tmp_path)
    approval_decision_file = _write_approval_decision(tmp_path, "approved")
...
def test_execution_gate_proposal_review_cli_missing_approval_decision_file_errors(tmp_path):
    session_file = _write_session(tmp_path)
    step_decision_file = _write_step_decision(tmp_path, "approved")
...
def test_execution_gate_proposal_review_cli_invalid_step_decision_errors(tmp_path):
    session_file = _write_session(tmp_path)
    approval_decision_file = _write_approval_decision(tmp_path, "approved")
