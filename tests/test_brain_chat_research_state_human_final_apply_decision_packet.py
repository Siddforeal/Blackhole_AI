...
from bugintel.core.brain_chat_research_state_human_final_apply_decision_packet import (
    ...
    build_human_final_apply_decision_packet_from_files,
    build_research_state_human_final_apply_decision_packet,
    load_json_object,
)
...
def _decisions(first_decision="approve-final-persistence-apply"):
    return {
...
def _build():
    return build_research_state_human_final_apply_decision_packet(_gate(), _decisions())
...
def test_constants():
    ...
    assert EXPECTED_GATE_STATUS == "ready-for-human-final-persistence-apply-review"
...
def test_ready_gate_and_human_decisions_build_packet():
    ...
    assert packet["human_final_apply_decision_count"] == 2
    assert packet["approved_final_apply_decision_count"] == 1
    assert len(packet["final_apply_decisions"]) == 2
    assert len(packet["approved_final_apply_items"]) == 1
...
def test_decision_records_preserve_review_context():
    item = _build()["final_apply_decisions"][0]
    ...
    assert item["decision"] == "approve-final-persistence-apply"
    assert item["decision_valid"] is True
    assert item["final_local_apply_preview_required"] is True
    assert item["persistent_write_allowed"] is False
    assert len(item["human_final_apply_decision_digest"]) == 64
...
def test_invalid_gate_kind_is_blocked():
    ...
    result = build_research_state_human_final_apply_decision_packet(gate, _decisions())

    assert result["decision_status"] == "blocked-invalid-final-persistence-apply-review-gate"
    assert "source-schema" in _categories(result, "source_findings")


def test_invalid_gate_status_is_blocked():
    gate = _gate()
    gate["gate_status"] = "blocked-invalid-final-persistence-apply-review-items"
...
def test_completed_gate_is_blocked():
    gate = _gate()
    gate["human_final_persistence_apply_decision_complete"] = True
...
def test_no_review_items_is_blocked():
    gate = _gate()
    gate["final_persistence_apply_review_items"] = []
    gate["final_persistence_apply_review_item_count"] = 0
...
def test_no_human_decisions_is_blocked():
    result = build_research_state_human_final_apply_decision_packet(_gate(), {"human_final_apply_decisions": []})

    assert result["decision_status"] == "blocked-no-human-final-apply-decisions"
    assert "decision-content" in _categories(result, "decision_findings")


def test_missing_review_decision_is_blocked():
    decisions = _decisions()
    decisions["human_final_apply_decisions"] = decisions["human_final_apply_decisions"][:1]

    result = build_research_state_human_final_apply_decision_packet(_gate(), decisions)

    assert result["decision_status"] == "blocked-invalid-human-final-apply-decisions"
    assert "decision-coverage" in _categories(result, "decision_findings")


def test_duplicate_decisions_are_blocked():
    decisions = _decisions()
    decisions["human_final_apply_decisions"][1]["final_persistence_apply_review_item_id"] = "FPARG-001"
...
def test_unknown_review_reference_is_blocked():
    decisions = _decisions()
    decisions["human_final_apply_decisions"][0]["final_persistence_apply_review_item_id"] = "FPARG-999"
...
def test_invalid_decision_value_is_blocked():
    result = build_research_state_human_final_apply_decision_packet(_gate(), _decisions("approve-write-now"))

    assert result["decision_status"] == "blocked-invalid-human-final-apply-decisions"
    assert "decision-value" in _categories(result, "decision_findings")


def test_no_approved_final_apply_decisions_is_blocked():
    result = build_research_state_human_final_apply_decision_packet(_gate(), _decisions("reject-final-persistence-apply"))

    assert result["decision_status"] == "blocked-no-approved-final-apply-decisions"
...
def test_packet_is_deterministic():
    ...
    assert first == second
    assert len(first["human_final_apply_decision_packet_digest"]) == 64
...
def test_builder_does_not_mutate_input():
    ...
    build_research_state_human_final_apply_decision_packet(gate, decisions)
...
def test_build_from_files_round_trip(tmp_path):
    ...
    result = build_human_final_apply_decision_packet_from_files(gate_file, decisions_file, output_file)

    assert result["decision_status"] == "ready-for-final-local-apply-preview"
