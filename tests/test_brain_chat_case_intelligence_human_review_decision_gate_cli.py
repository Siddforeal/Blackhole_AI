...
def test_human_review_decision_gate_cli_blocks_premature_approval(tmp_path):
    ...
    json_output = tmp_path / "human-review-decision-gate.json"
    markdown_output = tmp_path / "human-review-decision-gate.md"
    ...
    assert "Brain Chat Case Intelligence Human Review Decision Gate" in result.output
    assert "blocked-pending-effective-human-review" in result.output
...
def test_human_review_decision_gate_cli_changes_requested(tmp_path):
    session_file = _write_session(tmp_path)
...
def test_human_review_decision_gate_cli_rejected(tmp_path):
    session_file = _write_session(tmp_path)
...
def test_human_review_decision_gate_cli_no_artifacts_keeps_approval_blocked(tmp_path, monkeypatch):
...
def test_human_review_decision_gate_cli_missing_decision_file_errors(tmp_path):
    result = runner.invoke(
        app,
        [
            "brain-chat-case-intelligence-human-review-decision-gate",
...
def test_human_review_decision_gate_cli_invalid_decision_errors(tmp_path):
    session_file = _write_session(tmp_path)
