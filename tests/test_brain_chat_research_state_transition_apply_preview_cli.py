...
def _packet():
    ...
    return {
        "kind": "brain_chat_research_state_transition_apply_decision_packet",
        "source": "brain-chat-research-state-transition-apply-decision-packet",
        "target_name": "demo-target",
        "decision_status": "ready-for-research-state-transition-apply-preview",
...
def test_apply_preview_cli_writes_json(tmp_path):
    ...
    assert "Research-State Transition Apply Preview" in result.output
    assert "Saved research-state transition apply preview JSON" in result.output

    preview = json.loads(output_file.read_text(encoding="utf-8"))
    assert preview["kind"] == "brain_chat_research_state_transition_apply_preview"
...
def test_apply_preview_cli_prints_json_without_output(tmp_path):
    packet_file = tmp_path / "apply-decision-packet.json"
    packet_file.write_text(json.dumps(_packet()), encoding="utf-8")
    ...
    assert result.exit_code == 0, result.output
    assert '"kind": "brain_chat_research_state_transition_apply_preview"' in result.output
    assert '"apply_preview_ready": true' in result.output
...
def test_apply_preview_cli_missing_packet_file_exits(tmp_path):
    result = runner.invoke(
        app,
        [
            "brain-chat-research-state-transition-apply-preview",
    ...
    assert "Research-state transition apply decision packet JSON not found" in result.output
