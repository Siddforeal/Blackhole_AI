...
def test_human_review_decision_cli_blocks_premature_approval(tmp_path):
    ...
    json_output = tmp_path / "human-review-decision-output.json"
    markdown_output = tmp_path / "human-review-decision-output.md"
    ...
    assert "Brain Chat Case Intelligence Human Review Decision" in result.output
    assert "approved-for-human-case-review" in result.output
    assert "Effective approval granted" in result.output
    assert json_output.exists()
    assert markdown_output.exists()

    data = json.loads(json_output.read_text())
    assert data["kind"] == "brain_chat_case_intelligence_human_review_decision"
...
def test_human_review_decision_cli_changes_requested(tmp_path):
    session_file = _write_session(tmp_path)
...
def test_human_review_decision_cli_rejected(tmp_path):
    session_file = _write_session(tmp_path)
    ...
    assert "Stop this case-review path" in result.output


def test_human_review_decision_cli_no_artifacts_keeps_approval_ineffective(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 0
    assert "blocked-pending-briefing-review-gate" in result.output
    assert "Effective approval granted" in result.output
    assert "False" in result.output


def test_human_review_decision_cli_missing_decision_file_errors(tmp_path):
    result = runner.invoke(
        app,
        [
            "brain-chat-case-intelligence-human-review-decision-import",
...
def test_human_review_decision_cli_invalid_decision_errors(tmp_path):
    session_file = _write_session(tmp_path)
