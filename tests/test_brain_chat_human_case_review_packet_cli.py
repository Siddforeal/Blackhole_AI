...
def test_human_case_review_packet_cli_blocks_premature_approval(tmp_path):
    ...
    json_output = tmp_path / "human-case-review-packet.json"
    markdown_output = tmp_path / "human-case-review-packet.md"
    ...
    assert "Brain Chat Human Case Review Packet" in result.output
    assert "blocked-pending-human-review-decision-gate" in result.output
...
def test_human_case_review_packet_cli_changes_requested(tmp_path):
    session_file = _write_session(tmp_path)
...
def test_human_case_review_packet_cli_rejected(tmp_path):
    session_file = _write_session(tmp_path)
...
def test_human_case_review_packet_cli_no_artifacts_keeps_packet_blocked(tmp_path, monkeypatch):
...
def test_human_case_review_packet_cli_missing_decision_file_errors(tmp_path):
    result = runner.invoke(
        app,
        [
            "brain-chat-human-case-review-packet",
...
def test_human_case_review_packet_cli_invalid_decision_errors(tmp_path):
    session_file = _write_session(tmp_path)
