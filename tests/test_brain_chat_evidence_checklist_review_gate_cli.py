...
def test_evidence_checklist_review_gate_cli_blocks_missing_by_default(tmp_path):
    ...
    assert "Brain Chat Evidence Checklist Review Gate" in result.output
    assert "blocked" in result.output
    assert "7 evidence item(s) are still missing." in result.output
...
def test_evidence_checklist_review_gate_cli_needs_review_from_status_file(tmp_path):
    session_file = _write_session(tmp_path)
...
def test_evidence_checklist_review_gate_cli_ready_when_all_collected(tmp_path):
    session_file = _write_session(tmp_path)
    ...
    assert "Validation approval ready" in result.output
    assert "True" in result.output


def test_evidence_checklist_review_gate_cli_missing_status_file_errors(tmp_path):
    session_file = _write_session(tmp_path)
