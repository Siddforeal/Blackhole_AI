import copy
import json

from bugintel.core.brain_chat_research_state_human_final_apply_execution_decision_packet import (
    ALLOWED_HUMAN_FINAL_EXECUTION_DECISIONS,
    EXPECTED_GATE_KIND,
    EXPECTED_GATE_STATUS,
    EXPECTED_PACKET_KIND,
    READY_STATUS,
    build_human_final_apply_execution_decision_packet,
    build_human_final_apply_execution_decision_packet_from_files,
    load_json_object,
)


def _review_item(index=1, hypothesis_id="HYP-001"):
    item = {
        "final_apply_execution_review_item_id": f"FAERG-{index:03d}",
        "final_local_apply_preview_item_id": f"FLAP-{index:03d}",
        "human_final_apply_decision_id": f"HFAD-{index:03d}",
        "final_persistence_apply_review_item_id": f"FPARG-{index:03d}",
        "local_write_execution_packet_item_id": f"LWEP-{index:03d}",
        "write_execution_decision_id": f"WEDP-{index:03d}",
        "write_execution_review_item_id": f"WERG-{index:03d}",
        "local_write_packet_preview_item_id": f"LWPP-{index:03d}",
        "persistence_write_decision_id": f"PWRD-{index:03d}",
        "persistence_write_review_item_id": f"PWRG-{index:03d}",
        "source_preview_item_id": f"RSTPV-{index:03d}",
        "apply_decision_id": f"RSTAD-{index:03d}",
        "apply_review_item_id": f"RSTAR-{index:03d}",
        "operation_id": f"RSTO-{index:03d}",
        "transition_id": f"RST-{index:03d}",
        "decision_id": f"RSTD-{index:03d}",
        "hypothesis_id": hypothesis_id,
        "field_path": f"hypotheses.{hypothesis_id}.confidence",
        "operation_type": "local-proposed-hypothesis-confidence-update",
        "current_value": "medium",
        "proposed_value": "high",
        "final_apply_execution_review_summary": f"hypotheses.{hypothesis_id}.confidence: medium -> high",
        "human_final_apply_decision": "approve-final-persistence-apply",
        "human_final_apply_decision_reason": "Approved for later final local apply preview.",
        "human_final_apply_decision_actor": "human-reviewer",
        "human_final_apply_execution_decision_required": True,
        "human_final_apply_execution_decision_complete": False,
        "final_apply_execution_approved": False,
        "persistent_write_ready": False,
        "persistent_write_allowed": False,
        "research_state_transition_ready": False,
        "research_state_transition_allowed": False,
        "confidence_update_allowed": False,
        "hypothesis_mutation_allowed": False,
        "research_state_mutation_allowed": False,
        "execution_allowed": False,
        "runtime_execution_allowed": False,
        "source_final_local_apply_preview_item_digest": f"flap{index}" * 10,
        "source_human_final_apply_decision_digest": f"h{index}" * 32,
        "source_final_persistence_apply_review_item_digest": f"z{index}" * 32,
        "source_local_write_execution_packet_item_digest": f"x{index}" * 32,
        "source_write_execution_decision_digest": f"d{index}" * 32,
        "source_write_execution_review_item_digest": f"r{index}" * 32,
        "source_local_write_packet_preview_item_digest": f"l{index}" * 32,
        "source_persistence_write_decision_digest": f"p{index}" * 32,
        "source_persistence_write_review_item_digest": f"g{index}" * 32,
        "source_apply_preview_item_digest": f"a{index}" * 32,
        "source_apply_decision_digest": f"c{index}" * 32,
        "source_operation_digest": f"o{index}" * 32,
        "planning_only": True,
        "execution_state": "not_executed",
    }
    item["final_apply_execution_review_item_digest"] = (f"faerg{index}" * 16)[:64]
    return item


def _gate():
    items = [_review_item(1, "HYP-001"), _review_item(2, "HYP-002")]
    return {
        "kind": EXPECTED_GATE_KIND,
        "source": "brain-chat-research-state-final-apply-execution-review-gate",
        "target_name": "demo-target",
        "review_status": EXPECTED_GATE_STATUS,
        "source_final_local_apply_preview_digest": "fp" * 32,
        "source_human_final_apply_decision_packet_digest": "hp" * 32,
        "source_final_persistence_apply_review_gate_digest": "fg" * 32,
        "source_local_write_execution_packet_digest": "lp" * 32,
        "source_write_execution_decision_packet_digest": "ed" * 32,
        "source_write_execution_review_gate_digest": "eg" * 32,
        "source_local_write_packet_preview_digest": "lw" * 32,
        "source_persistence_write_decision_packet_digest": "wd" * 32,
        "source_persistence_write_review_gate_digest": "wg" * 32,
        "source_apply_preview_digest": "v" * 64,
        "source_apply_decision_packet_digest": "ad" * 32,
        "source_apply_review_gate_digest": "ar" * 32,
        "source_transition_packet_digest": "tp" * 32,
        "source_decision_digest": "d" * 64,
        "source_gate_digest": "g" * 64,
        "source_template_digest": "t" * 64,
        "source_update_digest": "u" * 64,
        "source_hypothesis_digest": "h" * 64,
        "source_feedback_digest": "f" * 64,
        "final_local_apply_preview_item_count": 2,
        "final_apply_execution_review_item_count": 2,
        "final_apply_execution_review_ready": True,
        "human_final_apply_execution_decision_required": True,
        "human_final_apply_execution_decision_complete": False,
        "persistent_research_state_write_ready": False,
        "persistent_research_state_write_allowed": False,
        "research_state_transition_ready": False,
        "allowed_human_final_execution_decisions": list(ALLOWED_HUMAN_FINAL_EXECUTION_DECISIONS),
        "final_apply_execution_review_items": items,
        "command_generation_allowed": False,
        "payload_generation_allowed": False,
        "package_installation_allowed": False,
        "execution_allowed": False,
        "runtime_execution_allowed": False,
        "network_interaction_allowed": False,
        "target_interaction_allowed": False,
        "evidence_collection_allowed": False,
        "validation_allowed": False,
        "confidence_update_allowed": False,
        "hypothesis_mutation_allowed": False,
        "selection_mutation_allowed": False,
        "investigation_plan_mutation_allowed": False,
        "research_state_mutation_allowed": False,
        "persistent_research_state_write_allowed": False,
        "report_submission_allowed": False,
        "vulnerability_confirmation_allowed": False,
        "planning_only": True,
        "execution_state": "not_executed",
        "final_apply_execution_review_gate_digest": "rg" * 32,
    }


def _decisions(first="approve-final-apply-execution", second="approve-final-apply-execution"):
    return {
        "human_final_apply_execution_decisions": [
            {
                "human_final_apply_execution_decision_id": "HFAED-001",
                "final_apply_execution_review_item_id": "FAERG-001",
                "decision": first,
                "decision_reason": "Approved for final apply execution packet review.",
                "decision_actor": "human-reviewer",
            },
            {
                "human_final_apply_execution_decision_id": "HFAED-002",
                "final_apply_execution_review_item_id": "FAERG-002",
                "decision": second,
                "decision_reason": "Approved for final apply execution packet review.",
                "decision_actor": "human-reviewer",
            },
        ]
    }


def _build():
    return build_human_final_apply_execution_decision_packet(_gate(), _decisions())


def _categories(packet, key):
    return {item["category"] for item in packet[key]}


def test_constants():
    assert EXPECTED_GATE_KIND == "brain_chat_research_state_final_apply_execution_review_gate"
    assert EXPECTED_GATE_STATUS == "ready-for-human-final-apply-execution-review"
    assert EXPECTED_PACKET_KIND == "brain_chat_research_state_human_final_apply_execution_decision_packet"
    assert READY_STATUS == "ready-for-final-apply-execution-packet"
    assert "approve-final-apply-execution" in ALLOWED_HUMAN_FINAL_EXECUTION_DECISIONS


def test_ready_gate_and_decisions_build_packet():
    packet = _build()

    assert packet["kind"] == EXPECTED_PACKET_KIND
    assert packet["decision_status"] == READY_STATUS
    assert packet["human_final_apply_execution_decision_complete"] is True
    assert packet["final_apply_execution_packet_required"] is True
    assert packet["final_apply_execution_packet_ready"] is False
    assert packet["persistent_research_state_write_ready"] is False
    assert packet["persistent_research_state_write_allowed"] is False
    assert packet["research_state_transition_ready"] is False
    assert packet["human_final_apply_execution_decision_count"] == 2
    assert packet["approved_final_apply_execution_decision_count"] == 2
    assert len(packet["human_final_apply_execution_decisions"]) == 2
    assert len(packet["approved_final_apply_execution_items"]) == 2
    assert packet["source_findings"] == []
    assert packet["safety_findings"] == []
    assert packet["review_item_findings"] == []
    assert packet["decision_findings"] == []


def test_packet_is_fail_closed():
    packet = _build()

    assert packet["persistent_research_state_write_allowed"] is False
    assert packet["confidence_update_allowed"] is False
    assert packet["hypothesis_mutation_allowed"] is False
    assert packet["selection_mutation_allowed"] is False
    assert packet["investigation_plan_mutation_allowed"] is False
    assert packet["research_state_mutation_allowed"] is False
    assert packet["execution_allowed"] is False
    assert packet["runtime_execution_allowed"] is False
    assert packet["target_interaction_allowed"] is False
    assert packet["evidence_collection_allowed"] is False
    assert packet["report_submission_allowed"] is False
    assert packet["vulnerability_confirmation_allowed"] is False


def test_decision_records_preserve_review_context():
    decision = _build()["human_final_apply_execution_decisions"][0]

    assert decision["human_final_apply_execution_decision_id"] == "HFAED-001"
    assert decision["final_apply_execution_review_item_id"] == "FAERG-001"
    assert decision["final_local_apply_preview_item_id"] == "FLAP-001"
    assert decision["operation_id"] == "RSTO-001"
    assert decision["hypothesis_id"] == "HYP-001"
    assert decision["field_path"] == "hypotheses.HYP-001.confidence"
    assert decision["current_value"] == "medium"
    assert decision["proposed_value"] == "high"
    assert decision["decision"] == "approve-final-apply-execution"
    assert decision["decision_valid"] is True
    assert decision["human_final_apply_execution_decision_complete"] is True
    assert decision["final_apply_execution_approved"] is True
    assert decision["final_apply_execution_packet_required"] is True
    assert decision["final_apply_execution_packet_ready"] is False
    assert decision["persistent_write_allowed"] is False
    assert len(decision["human_final_apply_execution_decision_digest"]) == 64


def test_invalid_gate_kind_is_blocked():
    gate = _gate()
    gate["kind"] = "wrong-kind"

    result = build_human_final_apply_execution_decision_packet(gate, _decisions())

    assert result["decision_status"] == "blocked-invalid-final-apply-execution-review-gate"
    assert "source-schema" in _categories(result, "source_findings")


def test_invalid_gate_status_is_blocked():
    gate = _gate()
    gate["review_status"] = "blocked-invalid-final-apply-execution-review-items"

    result = build_human_final_apply_execution_decision_packet(gate, _decisions())

    assert result["decision_status"] == "blocked-invalid-final-apply-execution-review-gate"
    assert "source-status" in _categories(result, "source_findings")


def test_not_ready_gate_is_blocked():
    gate = _gate()
    gate["final_apply_execution_review_ready"] = False

    result = build_human_final_apply_execution_decision_packet(gate, _decisions())

    assert result["decision_status"] == "blocked-invalid-final-apply-execution-review-gate"
    assert "source-readiness" in _categories(result, "source_findings")


def test_unsafe_gate_is_blocked():
    gate = _gate()
    gate["persistent_research_state_write_allowed"] = True

    result = build_human_final_apply_execution_decision_packet(gate, _decisions())

    assert result["decision_status"] == "blocked-invalid-final-apply-execution-review-gate"
    assert "source-safety" in _categories(result, "source_findings")


def test_no_review_items_is_blocked():
    gate = _gate()
    gate["final_apply_execution_review_items"] = []
    gate["final_apply_execution_review_item_count"] = 0

    result = build_human_final_apply_execution_decision_packet(gate, _decisions())

    assert result["decision_status"] == "blocked-invalid-final-apply-execution-review-gate"
    assert "source-content" in _categories(result, "source_findings")


def test_no_human_decisions_is_blocked():
    result = build_human_final_apply_execution_decision_packet(
        _gate(),
        {"human_final_apply_execution_decisions": []},
    )

    assert result["decision_status"] == "blocked-no-human-final-apply-execution-decisions"


def test_duplicate_human_decisions_are_blocked():
    decisions = _decisions()
    decisions["human_final_apply_execution_decisions"][1]["final_apply_execution_review_item_id"] = "FAERG-001"

    result = build_human_final_apply_execution_decision_packet(_gate(), decisions)

    assert result["decision_status"] == "blocked-invalid-human-final-apply-execution-decisions"
    assert "decision-coverage" in _categories(result, "decision_findings")


def test_unknown_review_item_decision_is_blocked():
    decisions = _decisions()
    decisions["human_final_apply_execution_decisions"][0]["final_apply_execution_review_item_id"] = "FAERG-999"

    result = build_human_final_apply_execution_decision_packet(_gate(), decisions)

    assert result["decision_status"] == "blocked-invalid-human-final-apply-execution-decisions"
    assert "decision-coverage" in _categories(result, "decision_findings")


def test_missing_decision_reason_is_blocked():
    decisions = _decisions()
    decisions["human_final_apply_execution_decisions"][0]["decision_reason"] = ""

    result = build_human_final_apply_execution_decision_packet(_gate(), decisions)

    assert result["decision_status"] == "blocked-invalid-human-final-apply-execution-decisions"
    assert "decision-rationale" in _categories(result, "decision_findings")


def test_invalid_decision_value_is_blocked():
    decisions = _decisions()
    decisions["human_final_apply_execution_decisions"][0]["decision"] = "approve-now"

    result = build_human_final_apply_execution_decision_packet(_gate(), decisions)

    assert result["decision_status"] == "blocked-invalid-human-final-apply-execution-decisions"
    assert "decision-value" in _categories(result, "decision_findings")


def test_no_approved_decisions_is_blocked():
    result = build_human_final_apply_execution_decision_packet(
        _gate(),
        _decisions("reject-final-apply-execution", "defer-final-apply-execution"),
    )

    assert result["decision_status"] == "blocked-no-approved-final-apply-execution-decisions"
    assert result["human_final_apply_execution_decision_complete"] is True
    assert result["approved_final_apply_execution_decision_count"] == 0


def test_packet_is_deterministic():
    first = _build()
    second = _build()

    assert first == second
    assert len(first["human_final_apply_execution_decision_packet_digest"]) == 64


def test_builder_does_not_mutate_inputs():
    gate = _gate()
    decisions = _decisions()
    original_gate = copy.deepcopy(gate)
    original_decisions = copy.deepcopy(decisions)

    build_human_final_apply_execution_decision_packet(gate, decisions)

    assert gate == original_gate
    assert decisions == original_decisions


def test_build_from_files_round_trip(tmp_path):
    gate_file = tmp_path / "final-apply-execution-review-gate.json"
    decisions_file = tmp_path / "human-final-apply-execution-decisions.json"
    output_file = tmp_path / "human-final-apply-execution-decision-packet.json"
    gate_file.write_text(json.dumps(_gate()), encoding="utf-8")
    decisions_file.write_text(json.dumps(_decisions()), encoding="utf-8")

    result = build_human_final_apply_execution_decision_packet_from_files(
        gate_file,
        decisions_file,
        output_file,
    )

    assert result["decision_status"] == READY_STATUS
    assert load_json_object(output_file) == result
