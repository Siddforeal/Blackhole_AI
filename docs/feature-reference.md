...
The apply review gate converts local transition operations into human review items. It does not apply confidence changes or write persistent research state.

A later human apply decision packet is required before any persistence stage can be considered.
...
The human persistence write decision packet combines a persistence write review gate with explicit human decisions. Approved decisions are prepared for a later local write packet preview.
...
The local write packet preview converts approved human persistence write decisions into local preview items. It shows proposed stored-state field updates without applying them.
...
The local write execution packet converts approved human write execution decisions into local packet records. It prepares records for a later final persistence apply review gate.

This artifact does not write stored research state, apply confidence changes, or mutate research-state records.

## v1.31.0 Final Persistence Apply Review Gate

The final persistence apply review gate converts local write execution packet records into human-reviewable final persistence apply review items. It prepares records for a later human final apply decision packet.

This artifact does not write stored research state, apply confidence changes, or mutate research-state records.

## v1.32.0 Human Final Apply Decision Packet

The human final apply decision packet combines final persistence apply review items with explicit human decisions. It prepares approved records for a later final local apply preview.
...
The human final apply execution decision packet combines final apply execution review items with explicit human final apply execution decisions. It prepares approved decisions for a later final apply execution packet.
