...
## brain-chat-validation-step-approval-decision-import

Import a local human reviewer decision for validation-step approval.

Examples:

    blackhole brain-chat-validation-step-approval-decision-import /tmp/case/validation-step-decision.json \
...
- step review readiness
- validation allowed flag
- effective step approval granted
- approved steps
- allowed next steps
- rejected next steps
- safety metadata
...
- local decision import only
- no side-effectful step approval is granted
- no validation execution
- no LLM provider calls
- no tool execution
- no browser execution
...
Build a local proposal for what a future validation-step execution gate would require.

Examples:

    blackhole brain-chat-validation-step-execution-gate-proposal /tmp/case/validation-step-decision.json \
      --approval-decision-file /tmp/case/approval-decision.json
...
Proposal statuses:
    ...
    ready-for-human-execution-gate-design
...
- approved steps
- proposed execution gate requirements
- proposed runtime guards
- rejected actions
- safety metadata
...
Build a local deterministic research hypothesis packet from user-provided local research sources.

Examples:

    blackhole brain-chat-research-hypothesis-packet \
      --sources-file /tmp/case/research-sources.json
...
With Markdown and JSON export:
    ...
    blackhole brain-chat-research-hypothesis-packet \
      --sources-file /tmp/case/research-sources.json \
      --target-name demo.local \
      --output-file /tmp/case/research-hypothesis-packet.md \
      --json-output /tmp/case/research-hypothesis-packet.json

Packet statuses:

- blocked-pending-ready-research-source-packet
...
- local deterministic hypothesis packet only
- no web browsing
- no network interaction
