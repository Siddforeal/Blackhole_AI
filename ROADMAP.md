...
> Historical note: the versioned sections below describe the original early roadmap. Many items listed as planned were later implemented. See **Current Direction After v1.30.0** for the active roadmap.
...
## Current Direction After v1.30.0

The current pipeline now has a local write execution packet for approved human write execution decisions. The next stage is a final persistence apply review gate. Stored-state writes remain disabled until a later separate final apply path is intentionally introduced and reviewed.
