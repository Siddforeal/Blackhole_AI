...
> Historical note: the versioned sections below describe the original early roadmap. Many items listed as planned were later implemented. See **Current Direction After v1.29.0** for the active roadmap.
...
## Current Direction After v1.29.0

The current pipeline now has a human write execution decision packet for write execution review gate items. The next stage is a local write execution packet or write apply preview. Stored-state writes remain disabled until a later separate write path is intentionally introduced and reviewed.
